...
public class Main {
    public static void main(String []args){
        ...
        while (sc.hasNextLine()) {
            ...
            if (g != null) {
                arvore.inserir(g); //insere na árvore binária (chave: name)
            }
        }
        
        //agora, vamos começar a ler os nomes e printar os caminhos
        //printei tudo no arquivo log tbm
        try (PrintWriter logWriter = new PrintWriter("885761_arvoreBinaria.txt")) {
            while (sc.hasNextLine()) {
    ...
    static class ArvoreBinaria {
        ...
        private Node inserirRec(Node no, Game novoJogo) {
            if (no == null) {
